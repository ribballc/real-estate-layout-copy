...
import { Mail, Clock, Phone, MapPin, Instagram, Facebook, Send, Check } from 'lucide-react';
  ...
  return (
    <section id="contact" className="site-section relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none opacity-[0.03] bg-accent" style={{
        maskImage: 'radial-gradient(ellipse 80% 60% at 20% 80%, black 0%, transparent 60%)',
        WebkitMaskImage: 'radial-gradient(ellipse 80% 60% at 20% 80%, black 0%, transparent 60%)',
      }} />
      <div className="site-container max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <SiteFadeIn>
            <div>
              <p className="text-[11px] uppercase tracking-[0.25em] text-[var(--site-primary)] font-semibold mb-3">Contact</p>
              ...
              <div className="space-y-5 mb-8">
                {contactItems.map((item, i) => (
                  <div key={i} className="flex items-start gap-4 group">
                    <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0 group-hover:bg-white/15 transition-colors duration-300">
              ...
              <div className="flex gap-3">
                {instagram && (
                  <a href={instagram} target="_blank" rel="noopener noreferrer" className="site-tap-target min-w-[44px] min-h-[44px] w-11 h-11 rounded-xl bg-surface-2 hover:bg-surface-3 hover:scale-105 flex items-center justify-center transition-all duration-300">
                    <Instagram className="w-4 h-4 text-white/50" />
                  </a>
                )}
                {facebook && (
                  <a href={facebook} target="_blank" rel="noopener noreferrer" className="site-tap-target min-w-[44px] min-h-[44px] w-11 h-11 rounded-xl bg-surface-2 hover:bg-surface-3 hover:scale-105 flex items-center justify-center transition-all duration-300">
                    <Facebook className="w-4 h-4 text-white/50" />
                  </a>
                )}
                {tiktok && (
                  <a href={tiktok} target="_blank" rel="noopener noreferrer" className="site-tap-target min-w-[44px] min-h-[44px] w-11 h-11 rounded-xl bg-surface-2 hover:bg-surface-3 hover:scale-105 flex items-center justify-center transition-all duration-300">
          ...
          <SiteFadeIn delay={150}>
            <div className="space-y-6">
              {/* Message us form */}
              <div className="w-full bg-surface-2 border border-white/10 rounded-2xl p-6 hover:border-white/15 shadow-glass transition-all duration-500">
                ...
                {leadSent ? (
                  ...
                  <form onSubmit={handleLeadSubmit} className="space-y-3">
                    <input
                      ...
                      className="w-full h-11 px-4 rounded-xl bg-surface-1 border border-white/10 text-white placeholder:text-white/40 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50"
                      maxLength={200}
                      required
                    />
                    <input
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 rounded-xl bg-surface-1 border border-white/10 text-white placeholder:text-white/40 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 resize-none"
                      maxLength={2000}
                      required
                    />
                    ...
                    <button
                      ...
                      className="site-tap-target w-full py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 bg-surface-2 text-white hover:bg-surface-3 border border-white/10 transition-colors disabled:opacity-50"
                    >
              ...
              {/* Book Online card */}
              <div className="w-full bg-surface-2 border border-white/10 rounded-2xl p-8 text-center hover:border-white/15 shadow-glass transition-all duration-500">
                <div className="w-14 h-14 rounded-2xl bg-surface-1 flex items-center justify-center mx-auto mb-6">
                ...
                <a
                  href={slug ? `/site/${slug}/book` : "#"}
                  className={`block w-full h-14 px-8 rounded-xl bg-[var(--site-primary)] text-white text-[15px] font-semibold tracking-[-0.01em] hover:opacity-90 active:scale-[0.98] transition-all duration-150 flex items-center justify-center ${slug ? 'book-now-link' : ''}`}
                >
                  Schedule Now
                </a>
              </div>
