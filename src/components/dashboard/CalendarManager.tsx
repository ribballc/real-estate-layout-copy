...
import {
  ...
  DollarSign, FileText, X, Calendar as CalendarIcon,
} from "lucide-react";
            ...
            {days.days.map(day => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <span className={`font-medium ${today ? "text-accent" : "text-white/70"} ${selected ? "text-accent font-bold" : ""}`}>
                    {format(day, "d")}
                  </span>
                  ...
                  {selectedBookings.map(b => (
                    <div key={b.id} className="rounded-lg border border-white/10 bg-white/[0.03] p-3.5 space-y-2 hover:border-white/20 transition-colors">
                      ...
                      <div className="space-y-1.5 text-xs">
                        ...
                        <div className="flex items-center gap-2 text-white/50">
                          <Clock className="w-3 h-3" /> {b.booking_time} · {b.duration_minutes}min
                        </div>
                        {b.customer_email && (
                          <div className="flex items-center gap-2 text-white/50">
                        ...
                        {b.service_price > 0 && (
                          <div className="flex items-center gap-2 text-white/50">
                            <DollarSign className="w-3 h-3" /> ${b.service_price}
                          </div>
                        )}
                        {b.notes && (
                          <div className="flex items-start gap-2 text-white/50">
                            <FileText className="w-3 h-3 mt-0.5" /> {b.notes}
                          </div>
                        )}
                      </div>
